...
class Users extends Component {
	...
	getCategories = async () => {
		...
		response = await apiCall('POST', 'getAllSubcategories', reqData);
		console.log("getAllCateogies", response)

		let categories = response.data.categories;
		console.log("categories", categories)
		this.setState({ categories: categories, total_categories: response.data.total_categories });
	}

	addCategory = () => {
		this.props.history.push(process.env.PUBLIC_URL + '/sub-categories/addSubcategory');
	}
	...
	delete = async (category) => {
		...
		let message = 
		[
			`Delete ${category.sub_categories_name} ? `,
		].join('')
		if (await confirmBox(title, message)) {
			...
			let response = await apiCall('POST', 'deleteSubCateogy', reqData);
	...
	editCategory = async (category) => {
		...
		this.props.history.push(process.env.PUBLIC_URL + `/sub-categories/edit/${category.id}`)
	}
	...
	CategoryRow = (category, index) => {
		return (
			< tr key={index} >
				...
				<td className="text-center align-middle">{category.sub_categories_name}</td>
				<td className="text-center align-middle">{category.name}</td>
				<td className="text-center align-middle">{moment(category.createdAt * 1000).format('DD MMM, YYYY')}</td>
				<td className="text-center align-middle">{moment(category.modifiedAt * 1000).format('DD MMM, YYYY')}</td>
				<td className="text-center align-middle">
					<span className="fa fa-edit mx-2" title="Edit User" onClick={() => this.editCategory(category)}></span>
					<span className="fa fa-trash mx-2" title="Delete user" onClick={() => this.delete(category)}></span>
				</td>
			</tr >
		)
	}
					...
					<Col xl={12}>
						<Card>
							<CardHeader>
								...
								<button className="chatoutbtn btn float-right" onClick={() => this.addCategory()}>Add New Sub Category</button>
							</CardHeader>
							<CardBody className="pb-0">
								<Row className="align-items-right d-flex">
									...
									<Col sm="6" md="10" xs className="mb-xl-0 float-right justify-content-end d-flex">
										<Input type="text" placeholder={`Search by Sub Category name`}
											className="search"
								...
								<Table bordered striped size="sm" className="CategoryTable">
									<thead>
										<tr>
											...
											<th scope="col" className="text-center align-middle">Sub Category Name</th>
						...
						<Row>
							<Col>
								<ReactPaginate
									pageCount={Math.ceil(this.state.total_categories / this.state.limit)}
